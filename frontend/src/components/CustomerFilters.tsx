...
export const CustomerFilters = ({ onFilterChange }: CustomerFiltersProps) => {
  ...
  const { data: billingGroups } = useQuery({
    queryKey: ["billing-groups-filter"],
    queryFn: async () => {
    ...
    <Card>
      <CardContent className="pt-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            <Label className="text-xs text-muted-foreground">Billing Group</Label>
            <Select 
              value={filters.billingGroupId || ""} 
              onValueChange={(value) => handleFilterUpdate("billingGroupId", value)}
            >
              <SelectTrigger>
          ...
          <div className="space-y-2">
            <Label className="text-xs text-muted-foreground">Min Due Amount</Label>
            <Input
              type="number"
              step="100"
